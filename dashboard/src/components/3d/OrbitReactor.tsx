...
function DependencyGraph({ currentStage, results }: { currentStage: number, results?: any }) {
  ...
  useFrame((state, delta) => {
    ...
    // Ambient breathing rotation — always present, very slow
    ...
    testNodes.forEach((node, i) => {
      ...
      if (currentStage >= 4 && currentStage < 6) {
        // Scene 4/5: reduction — non-targets fade
        if (node.isTarget) {
        ...
        // Ambient state — all visible with subtle breathing
      ...
      <Sphere ref={centerNodeRef} args={[0.5, 32, 32]} position={[0, 0, 0]}>
        ...
        <Html distanceFactor={25} center position={[0, 0.8, 0]}>
          <div className="bg-transparent px-2 py-0.5 whitespace-nowrap pointer-events-none opacity-100">
            <span className="text-[10px] font-mono font-bold text-[#1E1E1E] drop-shadow-md">_fmt()</span>
          </div>
        </Html>
      </Sphere>

      {/* REPOSITORIES & TRAVERSAL PATHS */}
      {REPOSITORIES.map((repo, i) => (
        <group key={repo.id}>
          <Line
            ...
            opacity={0.8}
          />
          <Sphere ref={(el) => { repoRefs.current[i] = el; }} args={[0.3, 32, 32]} position={repo.pos}>
...
export function OrbitReactor({ currentStage = -1, results }: { currentStage?: number, results?: any }) {
  return (
    <div className="w-full h-full bg-transparent relative overflow-hidden flex items-center justify-center">
      <Canvas camera={{ position: [0, 0, 25], fov: 45 }}>
        ...
        <DependencyGraph currentStage={currentStage} results={results} />
